...
# Crear subredes
SUBNET_PUBLIC_1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.207.1.0/24" --availability-zone "$REGION"a \
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=Public1}]' \
  --query 'Subnet.SubnetId' --output text)
echo "Subred pública 1 creada: $SUBNET_PUBLIC_1"

SUBNET_PUBLIC_2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.207.2.0/24" --availability-zone "$REGION"b \
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=Public2}]' \
  --query 'Subnet.SubnetId' --output text)
echo "Subred pública 2 creada: $SUBNET_PUBLIC_2"

SUBNET_PRIVATE_1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.207.3.0/24" --availability-zone "$REGION"a \
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=Private1}]' \
  --query 'Subnet.SubnetId' --output text)
echo "Subred privada 1 creada: $SUBNET_PRIVATE_1"

SUBNET_PRIVATE_2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.207.4.0/24" --availability-zone "$REGION"b \
...
INSTANCE_POSTGRES=$(aws ec2 run-instances \
  ...
  --private-ip-address 10.207.3.100 \
